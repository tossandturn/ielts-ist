...
- AI Coach text and screenshot explanations use the server-only `AI_GATEWAY_*` provider when configured (`ai.ieltsist.com`, GPT-5.5 with `reasoning_effort=xhigh`), then the configured DashScope/Qwen channel through `COACH_AI_*` (falling back to `DASHSCOPE_*`). When a Qwen key is configured, do not retry Coach requests through the legacy `OPENAI_*` gateway. Never return raw provider errors, balance messages, response bodies, endpoints, or credentials to students.
