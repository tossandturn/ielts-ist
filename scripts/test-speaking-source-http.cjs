const assert=require('node:assert/strict'),fs=require('node:fs/promises'),path=require('node:path'),os=require('node:os'),net=require('node:net'),{spawn}=require('node:child_process')
async function main(){
 ...
 try{
  ...
  assert.ok(payload,'isolated server ready');assert.equal(payload.speakingSets.length,55)
  const pages=new Set(payload.speakingSets.map(s=>JSON.stringify([s.sourceUrl,s.speakingPageImages])));assert.equal(pages.size,55)
  const repaired=payload.speakingSets.filter(s=>s.sourceReviewStatus==='ai-source-verified');assert.equal(repaired.length,52)
  ...
  const header={'X-STEMist-Catalog':'native-topics-v1'},catalog=await (await fetch(base+'/api/native/ielts/catalog',{headers:header})).json();assert.equal(catalog.speakingSets.length,201)
  for(const item of repaired){const r=await fetch(base+'/api/native/ielts/tasks/speaking/'+item.id,{headers:header});assert.equal(r.status,200);const detail=(await r.json()).task;assert.equal(detail.id,item.id);assert.equal(detail.part2,item.part2);assert.equal(detail.formalProgressEligible,false);assert.ok(detail.speakingPageImages.length);assert.doesNotMatch(detail.part2,/You will have to talk|You can make some notes/)}
  console.log(JSON.stringify({status:'PASS',originalSourcePages:55,recoveredAdditionalPages:52,sharedNativeSpeaking:201,oldPublishedIdsPreserved:true,isolatedDatabase:true}))
 }finally{
